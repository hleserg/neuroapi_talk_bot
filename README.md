...
- 🎤 Распознавание голосовых сообщений с локальной моделью Whisper Medium
...
### Распознавание речи с Whisper Medium

Бот использует локальную модель OpenAI Whisper Medium для распознавания голосовых сообщений:
- **Высокое качество**: Превосходное распознавание русской речи
- **Локальная обработка**: Полная конфиденциальность, без отправки данных в облако
- **Быстрая работа**: Оптимизировано для GPU ускорения
- **Независимость**: Не зависит от внешних API

**Особенности:**
- Автоматическое определение языка (русский/английский)
- Поддержка различных аудио форматов (OGG, MP3, WAV, M4A)
- GPU ускорение при наличии NVIDIA карты
- Детерминистичные результаты для повторяемости

**Требования к системе:**
- 6GB RAM (минимум)
- 3GB свободного места для модели
- NVIDIA GPU (рекомендуется для ускорения)

Подробная документация: [WHISPER_SETUP.md](WHISPER_SETUP.md)
...
├── whisper_service/         # Сервис распознавания речи (Whisper Medium)
│   ├── Dockerfile           # Docker образ для Whisper сервиса
│   ├── main.py              # FastAPI приложение Whisper
│   ├── requirements.txt     # Зависимости Whisper сервиса
│   └── test_whisper.py      # Тестовый скрипт для проверки сервиса
├── logs/                    # Директория для логов
├── OCR_SETUP.md            # Документация по OCR
├── KANDINSKY_SETUP.md      # Документация по Kandinsky
├── WHISPER_SETUP.md        # Документация по Whisper
└── README.md               # Этот файл
```
